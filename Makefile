...
GRAPH_SRC = $(SRCDIR)/graph.cu
COMMON_SRC = $(SRCDIR)/common.cu
...
# main: $(MAIN_OBJ) $(BFS_OBJ) $(BFS_KERNELS_OBJ) $(GRAPH_OBJ) $(COMMON_OBJ)
main: $(MAIN_OBJ) $(BFS_OBJ) $(BFS_KERNELS_OBJ) $(CC_OBJ) $(CC_KERNELS_OBJ) $(SSSP_OBJ) $(SSSP_KERNELS_OBJ) $(PR_OBJ) $(PR_KERNELS_OBJ) $(GRAPH_OBJ) $(COMMON_OBJ)
	$(NVCC) $(NVCCFLAGS) $^ -o $@

$(OBJDIR)/main.o: $(MAIN_SRC)
	$(NVCC) $(NVCCFLAGS) -c $< -o $@

$(OBJDIR)/graph.o: $(GRAPH_SRC)
	$(NVCC) $(NVCCFLAGS) -I$(SRCDIR) -c $< -o $@

$(OBJDIR)/common.o: $(COMMON_SRC)
	$(NVCC) $(NVCCFLAGS) -I$(SRCDIR) -c $< -o $@

$(OBJDIR)/timer.o: $(TIMER_SRC)
	$(NVCC) $(NVCCFLAGS) -c $< -o $@

# BFS
$(OBJDIR)/bfs.o: $(BFS_SRC)
	$(NVCC) $(NVCCFLAGS) -I$(BFS_SRCDIR) -c $< -o $@

$(OBJDIR)/bfs_kernels.o: $(BFS_KERNELS_SRC)
	$(NVCC) $(NVCCFLAGS) -I$(BFS_SRCDIR) -c $< -o $@
...
$(OBJDIR)/cc_kernels.o: $(CC_KERNELS_SRC)
	$(NVCC) $(NVCCFLAGS) -I$(CC_SRCDIR) -c $< -o $@

# SSSP
$(OBJDIR)/sssp.o: $(SSSP_SRC)
	$(NVCC) $(NVCCFLAGS) -I$(SSSP_SRCDIR) -c $< -o $@

$(OBJDIR)/sssp_kernels.o: $(SSSP_KERNELS_SRC)
	$(NVCC) $(NVCCFLAGS) -I$(SSSP_SRCDIR) -c $< -o $@
...
$(OBJDIR)/pr_kernels.o: $(PR_KERNELS_SRC)
	$(NVCC) $(NVCCFLAGS) -I$(PR_SRCDIR) -c $< -o $@
