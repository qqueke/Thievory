...
void PR32_PUSH(string filePath, double memAdvise, uint32 nRuns) {
  ...
  int device = 0; // Selected device
  uint32 k =
      2; // Multiple of SMs to choose for the grid dimension (to be adjusted)
  ...
  GPUAssert(cudaMalloc(&graph->d_delta,
                       (*graph->numVertices + 1) * sizeof(*graph->d_delta)));
  GPUAssert(cudaMalloc(&graph->d_residual,
                       (*graph->numVertices + 1) * sizeof(*graph->d_residual)));
  ...
  for (int test = 0; test < nRuns; test++) {
    ...
    for (uint32 i = 0; i < *graph->numVertices; i++) {
      ...
      h_delta[i] = (double)((1.0f - ALPHA) * ALPHA /
                            (graph->h_offsets[i + 1] - graph->h_offsets[i]));
      h_residual[i] = (double)0.0f;
    }
    cudaMemcpy(graph->d_valuesPR, graph->h_valuesPR,
               *graph->numVertices * sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(graph->d_delta, h_delta, *graph->numVertices * sizeof(double),
               cudaMemcpyHostToDevice);
    cudaMemcpy(graph->d_residual, h_residual,
               *graph->numVertices * sizeof(double), cudaMemcpyHostToDevice);
    ...
    *(graph->frontierSize) = thrust::reduce(
    ...
    while (*(graph->frontierSize)) {
      ...
      // This kernel will set the active vertices label to either Static or
      // On-Demand
      setStaticNDemandFrontiers<<<staticGrid, blockDim>>>(
          graph->numVertices, graph->d_frontier, graph->d_staticFrontier,
          graph->d_demandFrontier, graph->d_inStatic);
      ...
      *(graph->staticSize) =
      ...
      if (*(graph->staticSize) > 0) {
        ...
        setStaticList<<<staticGrid, blockDim>>>(
            graph->numVertices, graph->d_staticList, graph->d_staticFrontier,
            graph->d_prefixSum);
      }
      ...
      *(graph->demandSize) =
      ...
      if (*(graph->demandSize) > 0) {
        ...
        setDemandList<<<staticGrid, blockDim>>>(
            graph->numVertices, graph->d_demandList, graph->d_demandFrontier,
            graph->d_prefixSum);
      }
      ...
      if (*(graph->staticSize) > 0)
        setFrontier<<<staticGrid, blockDim, 0, staticStream>>>(
            graph->staticSize, graph->d_staticList, graph->d_frontier);
      ...
      PR32_Update_Values_PUSH<<<staticGrid, blockDim, 0, staticStream>>>(
