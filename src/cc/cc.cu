...
void CC32(std::string filePath, uint32 nRuns, uint32 nNeighborGPUs,
  ...
  auto asyncFrontierPolicy = thrust::cuda::par_nosync.on(graph->frontierStream);
  auto asyncStaticPolicy = thrust::cuda::par_nosync.on(graph->staticStream);
  auto asyncDemandPolicy = thrust::cuda::par_nosync.on(graph->demandStream);
      ...
      *(graph->demandSize) =
      ...
      if (*(graph->staticSize) > 0) {
        thrust::exclusive_scan(
            asyncStaticPolicy, graph->thurstStaticFrontier,
        ...
        // cudaStreamSynchronize(graph->frontierStream);
        //
        // CC32_Static_Kernel<<<staticGrid, blockDim, 0, graph->staticStream>>>(
      ...
      if (*(graph->demandSize) > 0) {
        cudaStreamSynchronize(graph->staticStream);
        thrust::exclusive_scan(
            asyncDemandPolicy, graph->thurstDemandFrontier,
        ...
        // cudaStreamSynchronize(graph->frontierStream);
        //
        // CC32_Demand_Kernel<<<gridDim, blockDim, 0, graph->demandStream>>>(
      ...
      if (*graph->frontierSize > 10 * graph->avgVertPerPart) {
        ...
        for (uint32 index = 0; index < partitionList.size(); index++) {
          ...
          for (uint32 j = 0; j < targetGPUQueue.size(); j++) {
            ...
            CC32_Filter_Kernel<<<staticGrid, blockDim, 0,
...
void CC64(std::string filePath, uint32 nRuns) { return; }
